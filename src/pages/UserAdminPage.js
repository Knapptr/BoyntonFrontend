      ...
      {users.length > 0 && (
        <Box mt={2}>
          ...
          {selectedUser && (
                ...
                <Box>
                  ...
                  <Stack component={"ul"} p={0}>
                    {selectedUser.sessions.map((s) => {
                      return (
                        <Stack key={`weekData-${s.weekNumber}-${s.username}`}component="li" direction="row" sx={{ listStyle: "none" }}>
                          <Box>Week {s.weekNumber - 1}</Box>
                        <Box>{s.cabinAssignment && `Cabin ${s.cabinAssignment}` || "Unassigned"}</Box>
                        </Stack>
                      );
                    })}
            ...
            />
        {selectedUserSession && <UserSessionView session={selectedUserSession}/> || <Typography>Select A Session</Typography>}
            </Grid>
            </Grid>
          )}
        </Box>
